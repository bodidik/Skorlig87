...
function ensureIndexes(db) {
  ...
  try {
    ...
    // unique + partialFilter: yalnızca string degerler indekslenir (null/undefined
    // atlanir). sparse+unique null ile sorun cikarir; partial filter daha guvenli.
...
/**
 * TÜRKÇE-DUYARSIZ ARAMA DESENİ.
 *
 * ⚠️ ARAMA TÜRKÇE ADLARDA FİİLEN ÇALIŞMIYORDU. Sorgu ve kayıt ham hâlde
 * karşılaştırılıyordu (Mongo tarafında `new RegExp(q,"i")`), yani kullanıcı
 * adı AYNEN, tüm işaretleriyle yazmak zorundaydı. Ölçüldü (2026-08-02,
 * bellek içi Mongo):
 *
 ...
 *     ara("ali")    -> Ali     (yalnizca ASCII ad bulunuyor)
 *
 * Türkçe adların çoğu ş/ğ/ü/ö/ç/ı/İ taşıdığı için arkadaş arama pratikte
 * kırıktı: kullanıcı arkadaşını bulamıyor, hata da almıyor — boş liste.
 *
 * ⚠️ MİGRATION GEREKTİRMEYEN ÇÖZÜM SEÇİLDİ. Normalize bir alan yazıp ona
 * sormak, mevcut 800+ kaydın hepsi güncellenene kadar aramayı yarım bırakırdı
 * (üretim verisine migration çalıştırmıyoruz). Bunun yerine SORGU genişliyor:
 * her harf, Türkçe karşılıklarını da kapsayan bir sınıfa dönüşüyor. Kayıt
 * neyle yazılmışsa yazılmış olsun eşleşir.
 *
 * ⚠️ `i` BAYRAĞI TEK BAŞINA YETMEZ: `"İ".toLowerCase()` düz `i` değil,
 * `i` + U+0307 üretir — aynı tuzağa `normNick` ve `mac-durumu` da düşmüştü.
 * Bu yüzden `İ` sınıfa AÇIKÇA yazılıyor.
 */
const OZEL = new Set([".", "*", "+", "?", "^", "$", "{", "}", "(", ")", "|", "[", "]", "\\"]);
const TR_SINIF = {
  c: "cçCÇ", g: "gğGĞ", i: "iıİIi̇", o: "oöOÖ", s: "sşSŞ", u: "uüUÜ",
  "ç": "cçCÇ", "ğ": "gğGĞ", "ı": "iıİI", "ö": "oöOÖ", "ş": "sşSŞ", "ü": "uüUÜ", "İ": "iıİI",
};
function trAramaDeseni(q) {
  const kacisli = String(q || "").split("").map((h) => {
    /* ⚠️ BÜYÜK HARFLİ SORGU DA EŞLEŞMELİ. İlk yazımım yalnızca küçük harf
     * anahtarlarına bakıyordu; ölçümde "Ismail" ve "SULE" boş dönüyordu.
     * Küçültme TÜRKÇE yerelli: "I" -> "ı" (İngilizcede "i" olurdu ve
     * ı/i ayrımı kaybolurdu). */
    const sinif = TR_SINIF[h] || TR_SINIF[h.toLocaleLowerCase("tr")];
    if (sinif) return `[${sinif}]`;
    /* Düzenli ifade özel karakteri ise kaçır. Karakter kümesi kullanıldı:
     * iç içe geçmiş kaçış katmanları burada bir kez bozulmuştu. */
    return OZEL.has(h) ? "\\" + h : h;
  }).join("");
  return new RegExp(kacisli, "i");
}
...
async function searchUsers(query, limit = 20, db) {
  ...
  if (db) {
    await ensureIndexes(db);
    /* ⚠️ Türkçe-duyarsız desen — kaçış işi trAramaDeseni içinde.
     * Eskiden ham `new RegExp(q,"i")` idi ve Türkçe adlar bulunamıyordu. */
    const re = trAramaDeseni(q);
    const docs = await db
      .collection(COLL)
  ...
  const { items } = await readBook();
  /* ⚠️ Dosya yedeği de AYNI deseni kullanmalı; ayrışırsa Mongo'lu ve
   * Mongo'suz ortamlar farklı sonuç verir ve kusur yalnızca birinde görünür. */
  const re = trAramaDeseni(q);
  const out = [];
  for (const u of items) {
    const ad = String(u?.nickname || "");
    const isim = String(u?.name || "");
    const id = String(u?.userId || "");
    if (re.test(ad) || re.test(isim) || re.test(id)) out.push(u);
...
async function listSegment1987(db) {
  ...
  const { items } = await readBook();
  return items.filter(
    // Tek kaynak: lib/premium.cjs uyeMi1987
    (u) => require("./premium.cjs").uyeMi1987(u)
  );
}
...
/**
 * KULLANICIYI TAMAMEN SİLER (hesap silme akışı).
 *
 * ⚠️ NEDEN VAR: bu depoda SİLME FONKSİYONU YOKTU. `DELETE /api/users/delete-account`
 * yalnızca `users.json` dosyasından çıkarıyordu; Mongo'daki profil satırı
 * duruyordu. Sonuç: hesabını silen kullanıcı aramalarda, takım kadrosunda ve
 * 1987 üye listesinde görünmeye devam ediyordu — ve Play Store'un "kullanıcı
 * verisini sil" şartı karşılanmıyordu.
 *
 * Bunu tests/hesap-silme.test.cjs yakaladı: kodda geçen her koleksiyonun ya
 * silme listesinde ya da gerekçeli muafiyet listesinde olmasını şart koşuyor.
 */
async function deleteUser(userId, db) {
  ...
  if (db) {
    try {
      await ensureIndexes(db);
      await db.collection(COLL).deleteOne({ userId: uid });
      ok = true;
    } catch (e) {
      // Sessiz kalmıyoruz: eksik silme bir uyum sorunudur.
      console.error("[users-store] kullanici silinemedi:", e?.message || e);
    }
  }
  ...
  if (needFile(db)) {
    try {
      ...
      const kalan = book.items.filter((u) => normId(u?.userId ?? u?.id) !== uid);
      if (kalan.length !== book.items.length) {
        await fsp.writeFile(FILE, JSON.stringify({ items: kalan }, null, 2), "utf8");
      }
    } catch (e) {
      console.error("[users-store] dosyadan silinemedi:", e?.message || e);
    }
  }
  return { ok };
}
...
module.exports = {
  // Sınama için: saf arama deseni (bkz. trAramaDeseni notu).
  _trAramaDeseni: trAramaDeseni,
  deleteUser,
  getUser,
  ensureUser,
  updateUser,
