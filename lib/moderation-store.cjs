...
/**
 * ⚠️ AYNA KAPALIYKEN DOSYAYA DÜŞÜLMEZ — YETKİ GERİ GELİYORDU.
 *
 * BULUNAN: Mongo boş dönünce KOŞULSUZ olarak dosyaya düşülüyor ve dosya
 * içeriği Mongo'ya TOHUMLANIYORDU. `SKORLIG_MODERATION_FILE_MIRROR=0` iken
 * `removeAdmin` aynayı tazelemiyor, yani dosya bayat kalıyor. Tek yöneticinin
 * yetkisi alınınca Mongo boşalıyor, okuma bayat dosyaya düşüyor ve yönetici
 * GERİ GELİYOR — üstelik tohumlama onu Mongo'ya yeniden YAZIYOR.
 *
 * ÖLÇÜLDÜ (tek yönetici, yetkisi alındı):
 *     ayna açık  → listAdmins: []                 temiz
 *     ayna kapalı→ listAdmins: [eski-yonetici]    yetki geri geldi + Mongo'ya geri yazıldı
 *
 * Kural: ayna kapalıysa dosyaya artık YAZILMIYOR demektir; oradan okumak
 * geçmişi diriltmektir. Aynı kural bu oturumda `lib/streak-store.cjs`'e de
 * uygulandı.
 *
 * ⚠️ HATA DURUMU AYRI: Mongo OKUNAMAZSA (istisna) dosyaya düşmek doğru —
 * "kimse yönetici değil" demek yönetimi kilitler. Ayrım, `mongoBos`
 * değişkeninde: boş sonuç ile okunamayan sonuç aynı şey değil.
 */
...
async function listBanned(db) {
  ...
  /* ⚠️ Ayna kapalıyken bayat dosya okunmaz — bkz. listAdmins. Burada yön
   * ters: bayat dosya KALDIRILMIŞ bir yasağı diriltirdi. Yine de aynı kural,
   * çünkü ayna kapalıyken dosya artık gerçeği temsil etmiyor. */
  if (mongoBos && !mirrorOn()) return [];
