...
function initRedis() {
  ...
  // TLS zorunlu sunucuya (Upstash gibi) `redis://` ile bağlanmak HATA ÜRETMEZ:
  // TCP açılır, sunucu TLS el sıkışması bekler, komutlar sessizce zaman aşımına
  // düşer. Tek belirti "maxRetriesPerRequest" hatasıdır ve sebep hiçbir yerde
  // görünmez (yaşandı — teşhisi saatler aldı). Upstash adresi için erken uyar.
  if (/^redis:\/\//.test(REDIS_URL) && /\.upstash\.io/i.test(REDIS_URL)) {
    console.error(
      "[rate-store] UYARI: Upstash TLS zorunlu tutar ama adres redis:// ile " +
        "basliyor — rediss:// (cift s) olmali. Baglanti sessizce calismayacak."
    );
  }
