...
async function fiksturKilidi(fixtureId, { oncekiDk = 0, db = null } = {}) {
  ...
  if (!st || typeof st !== "object") {
    ...
    try {
      // Indeksli tekil arama: eskiden TUM fikstur listesi cekilip bellekte
      // suzuluyordu (169 KB -> 271 bayt icin). bkz. fixtures-store.getOne
      const fx = await FixturesStore.getOne(fid, db);
