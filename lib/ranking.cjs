...
/**
 * NİTELİK EŞİĞİ — tek şanslı maç zirveye çıkarmasın.
 *
 * ⚠️ ÖLÇÜLDÜ (2026-07-30): Bu dosyanın başındaki tasarım notu "tek şanslı
 * tahmin zirve getirmez" diyor ve bu, MAÇ BAŞINA AZAMİ ~12 PUAN varsayımıyla
 * kalibre edilmişti. Ama puanlama çarpanları sonradan genişledi (settle2:
 * "üst sınır kalktı: çarpanlar değeri artırabilir") ve tek maçtan çıkabilecek
 * azami puan 56'ya yükseldi:
 *     sonuç 3×4.0 + kesin skor 12×2.5 + yan kalemler ×1.4 ≈ 53 (×1.05 ülke)
 *
 * Sonuç ölçüldü: 1 maç oynayıp 56 puan alan oyuncu rating 13.33 ile,
 * 60 maçta 9.80 ortalaması olan oyuncunun (9.70) ÜSTÜNE çıkıyordu.
 *
 * K'yı büyütmek çözmez: 1×56'nın 60×9.8'i geçmemesi için K > 513 gerekirdi.
 *
 * ÇÖZÜM — SERT BARAJ DEĞİL TAVAN: eşiğin altındaki oyuncu tabloda GÖRÜNÜR,
 * kendi ortalamasını ve puanını görür, ama rating'i havuz ortalamasını
 * AŞAMAZ. Yani zirveye çıkamaz, görünmez de olmaz (dosya başındaki "sert maç
 * barajı istemiyoruz" ilkesi korunuyor). Eşiğe ulaşınca tavan kalkar ve
 * gerçek ortalaması neyse oraya yerleşir.
 */
// Eşik 10 seçildi: 5'te tek 56 puanlık maç + 4 vasat maç hâlâ zirve
// getiriyordu (ortalama 12.8). 10 maçta bir uç değerin ortalamaya etkisi
// ~4.6 puana iner. Ücretsiz oyuncu günde ~2 maç oynayabildiği için (günlük
// taban 3 LC ÷ maç 3 LC) bu yaklaşık BİR HAFTA — aylık sezonda erişilebilir.
const RANK_MIN_PLAYED = Number(process.env.SKORLIG_RANK_MIN_PLAYED || 10);
...
function rankRows(rows) {
  ...
  for (const r of list) {
    ...
    // Eşiğin altında: havuz ortalamasını aşamaz (bkz. RANK_MIN_PLAYED).
    // Az örneklemin uç değeri zirveyi ele geçirmesin; oyuncu listede kalır.
    const nitelikli = played >= RANK_MIN_PLAYED;
    const rating = nitelikli ? hamRating : Math.min(hamRating, priorMean);

    r.avg = r2(avg);
    /* ⚠️ SIRALAMA HAM DEĞERLE, GÖSTERİM YUVARLANMIŞ.
     *
     * Eskiden `rating` 2 basamağa yuvarlanıp SIRALAMA da o değere göre
     * yapılıyordu. 1700+ oyunculu bir havuzda bu çok sayıda YAPAY EŞİTLİK
     * üretir (3.918 ve 3.912 → ikisi de 3.91) ve sıra, gerçek farkı olan
     * oyuncular arasında eşitlik bozucuya kalır. Uçtan uca koşumda görüldü:
     * -0.7 puanlı oyuncu -0.6 puanlının üstünde çıkıyordu.
     *
     * `_ratingRaw` yalnızca sıralama için; API yanıtında `rating` (yuvarlanmış)
     * kullanılmaya devam ediyor.
     */
    r._ratingRaw = rating;
    r.rating = r2(rating);
    // Şeffaflık: arayüz "sıralamaya girmek için N maç daha" diyebilsin.
    r.qualified = nitelikli;
    r.minPlayed = RANK_MIN_PLAYED;
    r.ratingWeight = w; // şeffaflık: tavana ulaşan oyuncu bunu görebilmeli
  }

  list.sort(
    (a, b) =>
      // Ham değerle: yuvarlama yapay eşitlik üretiyordu (bkz. _ratingRaw notu).
      (b._ratingRaw ?? b.rating ?? 0) - (a._ratingRaw ?? a.rating ?? 0) ||
  ...
  // Sıralama alanı yanıta sızmasın: istemciye giden `rating` yuvarlanmış olan.
  for (const r of list) delete r._ratingRaw;
...
function rankingMeta(rows) {
  ...
  return {
    ...
    minPlayedForRating: RANK_MIN_PLAYED,
...
module.exports = {
  ...
  computePriorMean,
  CONFIDENCE_K,
  RANK_MAX_PLAYED, RANK_MIN_PLAYED };
