...
/**
 * ÜLKE KARŞILAŞTIRMASI KANONİK OLMAK ZORUNDA.
 *
 * ⚠️ BULUNAN KUSUR (2026-08-03): ülke `ad()` ile ham karşılaştırılıyordu ve
 * `ad()` yalnızca küçültüp noktalama atıyor. Fikstür koleksiyonunda AYNI ülke
 * iki yazımla birden duruyor (ölçüldü: 86 ham değer → 77 kanonik, 9 ülke
 * bölünmüş):
 *
 *     Beyaz Rusya(13) + Belarus(16)        Galler(17) + Wales(8)
 *     Lüksemburg(2)   + Luxembourg(6)      Gürcistan(4) + Georgia(4)
 *     İrlanda Cumhuriyeti(2) + Ireland(17) ...
 *
 * Sonuç: aynı maçın iki kaydı FARKLI KOVAYA düşüyor ve hiç karşılaştırılmıyor
 * bile. ÖLÇÜLDÜ: 5 gerçek ikiz yalnızca bu yüzden kaçmış, örneğin
 *
 *     MK-GOMEL-...-VITEBS [Beyaz Rusya] Gomel - Vitebsk
 *     MK-GOMEL-...-FKVITE [Belarus]     Gomel - FK Vitebsk
 *
 * Zararı bu dosyanın kendi notunda yazılı: ikizin biri FT olup öderken öbürü
 * hiç uzlaşmıyor; o kayda tahmin yapan giriş bedelini ödeyip ödülünü hiç
 * alamıyor.
 *
 * ⚠️ AYNI İŞ BORU HATTINDA ZATEN DOĞRU YAPILIYORDU: `lib/fixture-priority.cjs`
 * içindeki `sameCountry` ham eşitlik tutmazsa `normalizeCountry`ye düşüyor;
 * kupon ülke süzgeci onu kullandığı için etkilenmemiş. Savunma bir yerde var,
 * öbüründe yok — bu oturumun en sık tekrarlayan kusur biçimi.
 *
 * Tanınmayan ülke `normalizeCountry`den KIRPILMIŞ hâliyle döner, yani bilinmeyen
 * adlar yine kendi başlarına gruplanır — eleme yapılmıyor.
 */
function ulkeAnahtari(c) {
  let k = c;
  try { k = require("./countries.cjs").normalizeCountry(c) || c; } catch { /* tablo yoksa ham ada düş */ }
  return ad(k);
}
...
/**
 * Başlama anı — DAKİKAYA yuvarlanmış epoch ms.
 *
 * ⚠️ ESKİDEN HAM STRING KESİLİYORDU (`kickoffISO.slice(0, 16)`) ve bu, farklı
 * yazım biçimi kullanan kaynaklar arasında ikizi KAÇIRIYORDU. Ölçüldü —
 * `data/fixtures.json` içinde iki biçim yan yana yaşıyor:
 *
 *     FDO → "2026-08-16T15:30Z"          (UTC)          259 kayıt
 *     MK  → "2026-08-16T18:30:00+03:00"  (yerel ofset)  1994 kayıt
 *
 * Aynı ANI gösteren bu iki damga, string olarak "15:30" ve "18:30" veriyor;
 * karşılaştırma tutmuyor ve aynı maç İKİ KAYIT olarak kalıyor. Kanıtlandı:
 *
 *     ikizMi(espn, mk) → false   (olması gereken: true)
 *
 * Bugün canlıda çakışma çıkmamasının sebebi savunmanın çalışması değil,
 * FDO ile MK'nın farklı ülkeleri kapsaması — yani şans. ESPN kaynağı Türkiye
 * için bağlanınca MK ile AYNI maçları yazacak ve kusur derhal görünür olurdu.
 *
 * ⚠️ SAAT DİLİMİ FARKI SORUN DEĞİL, AMAÇ: iki kaynak aynı anı farklı dilimde
 * yazabilir; karşılaştırılması gereken an, yazım değil.
 *
 * Damga çözülemezse ham stringe düşülüyor — eski davranış korunur, tanınmayan
 * biçimler yine kendi aralarında eşleşir.
 */
function baslangicAnahtari(iso) {
  const ham = String(iso || "");
  if (!ham) return "";
  const ms = Date.parse(ham);
  if (!Number.isFinite(ms)) return ham.slice(0, 16);
...
function ikizMi(a, b) {
  if (!a || !b) return false;
  const ka = baslangicAnahtari(a.kickoffISO);
  const kb = baslangicAnahtari(b.kickoffISO);
  if (!ka || ka !== kb) return false;
  if (ulkeAnahtari(a.country) !== ulkeAnahtari(b.country)) return false;
...
function ikizleriAyikla(list, bilinenIdler) {
  ...
  for (const f of items) {
    /* ⚠️ KOVA ANAHTARI DA `baslangicAnahtari` KULLANMAK ZORUNDA.
     *
     * Bir kez `ikizMi` düzeltilip BURASI ham `slice(0,16)` bırakıldı ve kusur
     * AYNEN sürdü: farklı yazım biçimindeki iki kayıt AYRI KOVAYA düşüyor,
     * `ikizMi` onlar için HİÇ ÇAĞRILMIYOR. Ölçüldü — `ikizMi(espn, mk)` true
     * dönerken `ikizleriAyikla([espn, mk])` iki kaydı da tutuyordu.
     *
     * Yani karşılaştırma fonksiyonunu düzeltmek yetmez; ön eleme onunla AYNI
     * eşitlik tanımını kullanmalı. */
    const k = `${baslangicAnahtari(f.kickoffISO)}|${ulkeAnahtari(f.country)}`;
...
module.exports = {
  ikizMi, ikizleriAyikla,
  _ad: ad, _varyantMi: varyantMi,
  /* ⚠️ BAKIM BETİĞİ DE AYNI KOVAYI KULLANMALI. scripts/ikiz-birlestir.cjs
   * kendi kovalamasını yazıyordu ve ham ülkeyle gruplandığı için düzeltmeden
   * sonra bile 5 çifti göremiyordu — kural kopyalanınca ayrışır. */
  _ulkeAnahtari: ulkeAnahtari,
};
