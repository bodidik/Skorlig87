...
/** `aynayiTazele`nin tanıdığı adlar — nöbetçi testi de bunu okuyor. */
const AYNA_ADLARI = ["groups", "mini", "friends", "tournaments", "duels"];

async function aynayiTazele(conn, hangi) {
  if (!mirrorOn()) return;
  /**
   * ⚠️ BİLİNMEYEN AD SESSİZ KALMAMALI — GERÇEK BİR KUSUR ÜRETTİ.
   *
   * Aşağıdaki zincirin `else` dalı yoktu: tanınmayan bir ad hiçbir dala
   * girmiyor ve fonksiyon hiçbir şey yapmadan dönüyordu. `removeUserFromSocial`
   * yazarken "links"/"requests"/"blocks" geçirmiştim — üçü de sessizce no-op.
   *
   * Sonuç zincirleme ve ağırdı: Mongo'dan silinen kayıt dosya aynasında
   * kalıyor, koleksiyon boşalınca okuma tarafındaki tohumlama onu Mongo'ya
   * GERİ YAZIYOR ve silinen arkadaşlık DİRİLİYORDU.
   *
   * Fırlatmıyoruz: ayna tazeleme yardımcı bir iş, bir yazma yolunu üretimde
   * patlatmaya değmez. Ama artık iz bırakıyor ve nöbetçi testi çağrıları
   * kaynaktan doğruluyor.
   */
  if (!AYNA_ADLARI.includes(hangi)) {
    console.error(
      `[social-store] aynayiTazele: bilinmeyen ayna adi "${hangi}" — ayna ` +
      `TAZELENMEDI. Gecerli adlar: ${AYNA_ADLARI.join(", ")}`
    );
    return;
  }
  try {
...
async function claimTournamentSettle(id, nowISO, db, ekAlanlar) {
  ...
  if (conn) {
    try {
       ...
       *
       * VARSAYILANLARDAN SONRA yayılıyor: zaman aşımı yolu `status:"voided"`
       * geçerek "settled" varsayılanını EZEBİLSİN diye. Sıra ters olsaydı
       * iptal edilen turnuva "settled" damgasıyla kalırdı. */
      const r = await conn.collection(COLL_TOURNAMENTS).updateOne(
        { id: tid, status: "open" },
  ...
  t.status = "settled";
  t.settledAt = damga;
  // Mongo yolundakiyle AYNI sıra: ek alanlar varsayılanları ezer.
  Object.assign(t, ekAlanlar || {});
  await saveTournaments(list, conn);
  return true;
}

/**
 * Katılımcı puanlarını HEDEFLİ yazar — yalnızca `participants[i].totalScore`.
 *
 * ⚠️ NEDEN `participants` DİZİSİ TOPTAN YAZILMIYOR: dizi `$set` ile
 * değiştirilseydi, çağıranın listeyi okuduğu an ile yazdığı an arasına giren
 * katılım SİLİNİRDİ — düzeltilmeye çalışılan kusurun aynısı, bu kez koleksiyon
 * yerine belge ölçeğinde. `arrayFilters` yalnızca adı geçen katılımcının tek
 * alanına dokunur; araya giren yeni katılımcı hiçbir filtreye uymaz ve
 * `totalScore` değeri 0 kalır (tahmini de olmadığı için doğrusu budur).
 *
 * ⚠️ POZİSYONEL `$` YETMEZ: ilk eşleşmeyi günceller, yani birden çok
 * katılımcının puanı tek işlemde yazılamaz.
 *
 * Puanlar `payouts` ile ÖRTÜŞMÜYOR: ödeme tablosu yalnızca ilk 3-4 sırayı
 * kapsıyor (n≥8 için PAYOUT_8PLUS 4 kalem), oysa `GET /api/tournaments/:code`
 * TÜM katılımcıları puanlarıyla döndürüyor.
 *
 * @param {Array<{userId:string,totalScore:number}>} skorlar
 */
async function setTournamentScoresAtomik(id, skorlar, db) {
  const tid = String(id || "");
  const liste = (Array.isArray(skorlar) ? skorlar : []).filter((s) => s && s.userId);
  if (!tid || !liste.length) return false;
  ...
  if (conn) {
    try {
      ...
      const filtreler = [];
      liste.forEach((s, i) => {
        set[`participants.$[p${i}].totalScore`] = Number(s.totalScore) || 0;
        filtreler.push({ [`p${i}.userId`]: s.userId });
      });
      await conn.collection(COLL_TOURNAMENTS).updateOne(
        { id: tid },
        { $set: set },
        { arrayFilters: filtreler }
      );
      await aynayiTazele(conn, "tournaments");
      return true;
    } catch (e) {
      // Puan kaybı para kaybı değil: ödeme `payouts` üzerinden yapıldı.
      console.error("[social-store] turnuva puanlari yazilamadi:", e?.message || e);
      return false;
    }
  }
  ...
  for (const s of liste) {
    const p = (t.participants || []).find((x) => x && x.userId === s.userId);
    if (p) p.totalScore = Number(s.totalScore) || 0;
  }
  await saveTournaments(list, conn);
  return true;
}
...
module.exports = {
  ...
  engelliMi,
  loadTournaments, saveTournaments, claimTournamentSettle, setTournamentScoresAtomik,
  joinTournamentAtomik, insertTournamentAtomik, setTournamentPredictionAtomik,
