...
/**
 * ⚠️ VARSAYILAN DEĞİŞTİ: MONGO VARSA AYNA KAPALI.
 *
 * Eskiden varsayılan koşulsuz açıktı (`?? "1"`), yani her snapshot yazımında
 * TÜM kitap dosyaya yeniden serileştiriliyordu. Bu dosya bugün 23.6 MB.
 *
 * ÖLÇÜLDÜ (1537 kayıt, gerçek veri):
 *     tek snapshot yazımı : 507 ms (medyan)
 *     bunun 373 ms'i      : JSON.stringify — OLAY DÖNGÜSÜNÜ BLOKLAR
 *     maç günü ~50 settle : ~26 sn toplam blokaj, üstelik dosya kilidi tutarak
 *
 * ⚠️ VE BU MALİYET KARŞILIKSIZ: Mongo varken dosya HİÇ OKUNMUYOR.
 * `getSnapshot` (satır ~88) ve `listSnapshots` `db` varsa doğrudan
 * koleksiyona gidiyor, dosyaya yalnızca `db` YOKKEN düşüyorlar. Yani ayna
 * yazma-only bir yandı.
 *
 * ⚠️ BAYAT AYNA RİSKİ YOK: bu modülde tohumlama (koleksiyon boşsa dosyadan
 * yükle) YOK — `lib/social-store.cjs`ten farkı bu. Orada aynayı kapatmak,
 * eski dosyanın Mongo'ya geri yazılmasına yol açabilirdi (bu oturumda bir kez
 * yaşandı: silinen arkadaşlık dirildi). Burada öyle bir yol yok.
 *
 * ⚠️ RİSK: Mongo tamamen kaybolursa artık dosya yedeği olmayacak. Kabul
 * edilebilir çünkü Render'da disk kalıcı değil — ayna zaten her deploy'da
 * siliniyordu, yani güvenilir bir yedek değildi. Yedek isteyen
 * `SKORLIG_MATCHRESULTS_FILE_MIRROR=1` ile açıkça açabilir.
 */
const FILE_MIRROR = (() => {
  const ham = process.env.SKORLIG_MATCHRESULTS_FILE_MIRROR;
  if (ham !== undefined && ham !== "") return String(ham) !== "0";
  return !process.env.MONGODB_URI; // Mongo yoksa dosya TEK kaynak — şart
})();
...
module.exports = {
  // Sınama için: ayna varsayılanı ortama göre değişiyor (bkz. FILE_MIRROR notu).
  _FILE_MIRROR: FILE_MIRROR,
  getSnapshot,
  listSnapshots,
  settledFixtureIds,
  upsertSnapshot,
