...
/**
 * Kupon bedeli DÖNEME GÖRE — tekli maç bedeliyle aynı oranda iner/çıkar.
 *
 * ⚠️ NEDEN (2026-08-08 lansman taraması): yukarıdaki gerekçe "kupon tek tek
 * oynamaktan UCUZ olmalı" der (10 < 8×3=24). Lansman tekli bedeli 1 LC'ye
 * indirince 8 maç tek tek 8 LC oldu, kupon 10 LC'de kaldı — ana ekranın
 * BİRİNCİL oyunu, tam lansman kitlesi için pahalı yola dönüştü ve kendi
 * değişmezini kırdı. Bedel artık tekli bedelin oranıyla ölçeklenir:
 * lansmanda ülke 10→3, Avrupa 15→5; dönem bitince kendiliğinden 10/15.
 * Kupon BELGESİ oluşturma anındaki bedeli saklar (katılım ve iade o
 * belgeden okunur) — dönem sınırında tutarlılık bozulmaz.
 */
function girisBedeli(tur) {
  const { macGirisBedeli, MAC_GIRIS_BEDELI_NORMAL } = require("./ekonomi.cjs");
  const normal = Number(GIRIS_BEDELI[tur] || 0);
  const oran = macGirisBedeli() / (MAC_GIRIS_BEDELI_NORMAL || 3);
  return Math.max(1, Math.round(normal * oran));
}

/**
 * KATILIM KİLİDİ — ilk maçtan kaç dakika ÖNCE kapanır.
 *
 * ⚠️ İlk maçın tam başlama anı DEĞİL. Sebep: fikstür saatleri dış kaynaklardan
 * geliyor ve gecikmeli/yanlış olabiliyor; maç fiilen başlamışken kupon hâlâ
 * "açık" görünürse sonucu bilerek katılmak mümkün olur. Bir saatlik tampon,
 * ilk dönemde kaynak oynaklığına karşı güvenli tarafta kalmayı sağlıyor.
 *
 * Tampon KISALTILIRSA bu risk geri gelir — düşürmeden önce fikstür saatlerinin
 * ne kadar güvenilir olduğunu ölç.
 */
const KILIT_ONCE_DK = Number(process.env.SKORLIG_KUPON_KILIT_DK || 60);
