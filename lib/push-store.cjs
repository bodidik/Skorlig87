...
/** Tüm kayıtlar (gönderim taraması için) — `{ items: { uid: {...} } }`. */
async function loadStore(db) {
  ...
  if (conn) {
    try {
      ...
      if (docs.length) {
        const items = {};
        for (const d of docs) items[d.userId || d.userIdLower] = strip(d);
        return { items };
      }
...
module.exports = { loadStore, saveStore, getUser, saveUser, removeUser, COLL, FILE };
