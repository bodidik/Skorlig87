 ...
 *
 * ⚠️ ANAHTARLAR ÖNCEDEN HESAPLANIYOR (süsle-sırala-çöz). Eskiden `priorityOf`
 * ve `koMs` KARŞILAŞTIRICININ İÇİNDE çağrılıyordu; sıralama O(n log n)
 * karşılaştırma yaptığı için 1771 fikstürde ~19.000 karşılaştırma × 2 çağrı
 * = ~38.000 hesap demekti — oysa öğe başına BİR kez yeterli.
 *
 * ÖLÇÜLDÜ (üretim verisi, 1771 fikstür): sortByPriority **2245 ms**.
 * `priorityOf` içinde `teamCountry` (iki takım için) ve tarih ayrıştırma var;
 * ikisi de karşılaştırma başına tekrarlanıyordu. Bu, olay döngüsü blokajı
 * düzeltildikten SONRA ana ekranın kalan en büyük maliyetiydi (4.1 sn).
 *
 * ⚠️ SIRALAMA SONUCU BİREBİR AYNI: aynı anahtarlar, aynı karşılaştırma
 * kuralı, ve JS `sort` ES2019'dan beri KARARLI — yani eşit anahtarlı
 * öğelerin göreli sırası da korunuyor. Gerçek veriyle doğrulandı.
 */
function sortByPriority(list, userCountry) {
  const dizi = list || [];
  /* Anahtarları bir kez hesapla; `i` kararlılığı ayrıca garantiye alıyor
   * (motor kararsız olsa bile sonuç değişmesin). */
  const susluk = dizi.map((it, i) => ({
    it,
    i,
    p: priorityOf(it, userCountry),
    ko: koMs(it),
  }));
