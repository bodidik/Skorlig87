...
function yukle() {
  ...
  for (const u of ulkeler) {
    ...
    /**
     * TAKMA ADLAR — yalnızca ARAMA indeksine girer, seçici listesine GİRMEZ.
     *
     * ⚠️ NEDEN AYRI ALAN: sağlayıcılar aynı kulübü farklı yazıyor. ESPN
     * "Istanbul Basaksehir" ve "Caykur Rizespor" derken katalog "Başakşehir"
     * ve "Rizespor" tutuyor; `cekirdek` önek yüzünden eşleştiremiyor
     * ("istanbul basaksehir" ≠ "basaksehir") ve `kanonikTakim` null dönüyordu.
     * Null dönmesi fikstürün ham sağlayıcı adıyla yazılması demek — ekranda
     * aksansız ad ve İKİZ fikstür riski.
     *
     * ⚠️ `teams` DİZİSİNE EKLEMEK YANLIŞ OLURDU: o dizi aynı zamanda
     * onboarding seçicisini besliyor (bkz. `takimlar`). Varyantı oraya
     * koymak aynı kulübü listede İKİ KEZ gösterirdi ve iki farklı ad seçen
     * taraftarlar ayrı sıralamalara düşerdi — bu modülün en başta önlemek
     * için yazıldığı kusurun ta kendisi.
     */
    const takma = u?.aliases && typeof u.aliases === "object" ? u.aliases : null;
    if (takma) {
      for (const [varyant, kanonik] of Object.entries(takma)) {
        const hedef = String(kanonik || "").trim();
        // Hedef gerçekten katalogda olmalı; yoksa takma ad hayalet ada işaret eder.
        if (!hedef || !kanonikler.includes(hedef)) continue;
        const a = anahtarla(varyant);
        if (a && !tamIx.has(a)) tamIx.set(a, hedef);
      }
    }
  }
