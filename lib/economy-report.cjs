...
function akis(kayitlar, sinirISO) {
  ...
  for (const t of kayitlar) {
    ...
    if (a > 0) giris[sebep] = Math.round((sayi(giris[sebep]) + a) * 10) / 10;
    else cikis[sebep] = Math.round((sayi(cikis[sebep]) + Math.abs(a)) * 10) / 10;
  }

  const topla = (o) => Math.round(Object.values(o).reduce((a, b) => a + b, 0) * 10) / 10;
  const tg = topla(giris);
  ...
  return {
    ...
    // 145:1 gibi bir oran "gider fiilen yok" demek. 1'e yakın olması hedef.
...
async function economyReport(db, gun = 7) {
  ...
  if (d7.girisCikisOrani === null && d7.toplamGiris > 0) {
    ...
    uyarilar.push(
      `Giriş/çıkış oranı ${d7.girisCikisOrani}:1 — gider kalemleri yetersiz.`
    );
  }
