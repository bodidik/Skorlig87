 ...
 *   });
 */
function _zamanAsimli(fn, ms, key) {
  return new Promise((resolve, reject) => {
    let bitti = false;
    const zamanlayici = setTimeout(() => {
      if (!bitti) {
        bitti = true;
        reject(new Error(`FILE_LOCK_TIMEOUT: ${key} ${ms}ms`));
      }
    }, ms);
    Promise.resolve().then(fn).then(
      (v) => { if (!bitti) { bitti = true; clearTimeout(zamanlayici); resolve(v); } },
      (e) => { if (!bitti) { bitti = true; clearTimeout(zamanlayici); reject(e); } }
    );
  });
}
...
function withFileLock(key, fn, timeoutMs = LOCK_TIMEOUT_MS) {
  const k = String(key);
  const prev = _tails.get(k) || Promise.resolve();

  _active++;
  const result = prev.then(
    () => _zamanAsimli(fn, timeoutMs, k),
    () => _zamanAsimli(fn, timeoutMs, k)
  );
  result.then(
    () => { _active--; },
  ...
  for (let i = 0; ; i++) {
    try {
      ...
      if (i >= deneme - 1) {
        ...
        /* ⚠️ SON ÇARE YOLU GEÇİCİ DOSYAYI TEMİZLEMİYORDU.
         * `writeJsonAtomic`in `catch`i yalnızca HATA fırlatıldığında unlink
         * yapıyor; bu dal başarıyla dönüyor ama `tmp` dosyası hâlâ diskte.
         * Her Windows EPERM zinciri bir çöp bırakıyordu — `bayat-tmp-temizle`
         * sonradan süpürsün diye VAR ama kaynağını kapatmak daha ucuz. */
        await fsp.unlink(tmp).catch(() => {});
        return;
      }
...
module.exports = { withFileLock, writeJsonAtomic, activeLockCount, atomikDusus, LOCK_TIMEOUT_MS };
