...
async function katilimEkle(katilim, db) {
  const conn = await getDbSafe(db);
  if (!conn) return { ok: false, reason: "NO_DB" };
  /* ⚠️ BENZERSİZ İNDEKS YOKSA KORUMA DA YOK — FAIL-OPEN İDİ.
   *
   * BULUNAN: `ensureIndexes` hatayı loglayıp geçiyordu. Ama bu fonksiyonun
   * atomikliği TAMAMEN `{kuponId, userIdLower}` benzersiz indeksine dayanıyor
   * (yukarıdaki "PARA KORUMASI" notu): indeks yoksa `insertOne` her çağrıda
   * başarılı olur ve 11000 hiç gelmez.
   *
   * ÖLÇÜLDÜ (koleksiyonda kopya kayıt bırakıp indeks kurulumunu düşürdüm):
   *     üç çağrı → [ok, ok, ok],  kayıt sayısı 3
   * Yani aynı oyuncu aynı kupona üç kez katılır; `routes/kupon.cjs` sırası
   * "önce tahsilat, sonra kayıt" olduğu için giriş bedelini ÜÇ KEZ öder ve
   * sonuçlandırmada ÜÇ KEZ ödül alır. Dosyanın kendi notu bunu yazıyor:
   * "Benzersizlik olmazsa aynı kişi iki kez ödeyip iki kez ödül alabilir."
   *
   * ⚠️ FAIL-CLOSED: koruma yoksa katılım yazılmıyor. Çağıran zaten
   * `!kayit.ok` durumunda ücreti İADE ediyor, yani oyuncu parasız kalmıyor —
   * yalnızca katılım gerçekleşmiyor. */
  if (!(await ensureIndexes(conn))) {
    console.error("[kupon-store] benzersiz indeks yok — katilim yazilmiyor (fail-closed)");
