...
async function bonus1987Tamamla(db, userId) {
  ...
  await ensureIndexes(db);
  const nowISO = new Date().toISOString();
  /* Bayrak, tamamlama filtresinden BAĞIMSIZ atılır: bonus tavandayken
   * (yeniden doğrulama, ya da 6-8 Ağustos arasında doğrulamış cüzdanlar)
   * aşağıdaki filtre eşleşmez ve bayrak sonsuza dek bayat kalırdı. */
  await db.collection(COLL_USERS).updateOne(
    { userIdLower: uidLower, is1987: { $ne: true } },
    { $set: { is1987: true, updatedAt: nowISO } }
  );
  const r = await db.collection(COLL_USERS).findOneAndUpdate(
    ...
    [{ $set: {
      ...
      /* ⚠️ CÜZDAN BAYRAĞI DA BURADA GÜNCELLENİR. Gerçek lansman akışı
       * "önce uygulamayı aç (cüzdan is1987:false doğar), SONRA kod gir".
       * Üyelik UsersStore'a yazılıyor ama summary `is1987`i CÜZDAN
       * belgesinden okuyor — bayrak bayat kalınca üyenin rozeti ve bonus
       * cebi ekranda HİÇ görünmüyordu (yolculuk testi 2. adım yakaladı). */
      is1987: { $literal: true },
      updatedAt: { $literal: nowISO },
    }}],
    { returnDocument: "after" }
