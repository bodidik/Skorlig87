...
async function saveAll(list, db) {
  let items = (Array.isArray(list) ? list : []).filter((f) => f && f.fixtureId != null);

  /* ⚠️ AYNI MAÇIN İKİ KEZ YAZILMASINI BURADA KESİYORUZ — bu dosya
   * koleksiyonun TEK YAZANI (bkz. yukarıdaki önbellek notu), yani tek
   * choke point. Üreticiye tek tek eklemek, birini unutmaya açık olurdu.
   *
   * ÖLÇÜLDÜ (2026-08-02, 1930 fikstür): aynı gerçek maç iki ad yazımıyla iki
   * ayrı kimlikte duruyordu — 115 çift. 23'ünde durum dosyası YALNIZCA
   * birinde vardı, yani ikizin biri ödeyip öbürü hiç uzlaşmıyordu; o kayda
   * tahmin yapan giriş bedelini ödeyip ödülünü hiç alamıyordu.
   *
   * ⚠️ DEPODA OLAN KAYIT HER ZAMAN KAZANIR: ona tahmin/havuz/düello bağlanmış
   * olabilir, düşürmek onları öksüz bırakırdı. bkz. lib/fikstur-ikiz.cjs
   * ⚠️ MEVCUT KOPYALARI TEMİZLEMİYOR: ikisi de bilindiği için ikisi de kalır.
   * Bu bilinçli — geçmişe dönük birleştirme ayrı bir karar. */
  try {
    const { ikizleriAyikla } = require("./fikstur-ikiz.cjs");
    const conn0 = await getDbSafe(db);
    let bilinen = new Set();
    if (conn0) {
      const mevcut = await conn0.collection(COLL)
        .find({}, { projection: { fixtureId: 1, _id: 0 } }).toArray().catch(() => []);
      bilinen = new Set(mevcut.map((x) => String(x.fixtureId)));
    }
    const { list: temiz, dusenler } = ikizleriAyikla(items, bilinen);
    if (dusenler.length) {
      console.warn(`[fixtures] ${dusenler.length} IKIZ fikstur yazilmadi (ayni mac, farkli ad yazimi)`);
      for (const d of dusenler.slice(0, 5)) console.warn(`  ${d.dusen} "${d.mac}" -> ${d.kalan} "${d.yerine}"`);
      items = temiz;
    }
  } catch (e) {
    // Ayıklama başarısızsa YAZMAYI DURDURMA: kopya, veri kaybından iyidir.
    console.error("[fixtures] ikiz ayiklama basarisiz, liste oldugu gibi yaziliyor:", e?.message || e);
  }
