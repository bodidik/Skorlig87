...
/**
 * KISMİ SONUÇLANDIRMANIN EKONOMİK TABANI.
 *
 * ⚠️ BULUNAN: ödül kademeleri `doğru / ÇÖZÜLEN` oranına bakıyor
 * (`const toplam = cozulen`), yani maçlar ertelendikçe "tam isabet" KOLAYLAŞIYOR
 * ama ödül aynı kalıyor. Kupon LC yakmak üzere ayarlanmışken bir noktadan sonra
 * LC BASMAYA başlıyor.
 *
 ...
 *
 * Eski eşik `0.5` ise ülke kuponunu 4 maçta, Avrupa kuponunu 3 maçta
 * sonuçlandırıyordu — ikisi de tam LC BASAN bölgede. Yani eşik, ekonominin
 * ters döndüğü noktanın YANLIŞ tarafındaydı.
 *
 * ⚠️ SABİT SAYI YAZMIYORUZ. Kademeler (`ODUL_KADEMELERI`) değişirse elle
 * yazılmış bir eşik sessizce bayatlar — bu oturumda elle tutulan listelerin
 * gerçeklikten ayrışması üç kez hata üretti. Taban, ödül tablosundan
 * TÜRETİLİYOR: LC basan en yüksek çözülen-sayısının bir üstü.
 *
 * Tek bir oran iki kupon türünü birden koruyamıyordu (ülke 5/8 = 0.625,
 * avrupa 5/6 = 0.834); türetme bunu da çözüyor.
 */
const _tabanOnbellek = new Map();
function ekonomikTaban(toplamMac, tur) {
  const anahtar = `${toplamMac}|${tur}`;
  if (_tabanOnbellek.has(anahtar)) return _tabanOnbellek.get(anahtar);

  const giris = Number(Kupon.GIRIS_BEDELI?.[tur]) || 0;
  let enYuksekBasan = 0;
  if (giris > 0 && typeof Kupon.beklenenOdul === "function") {
    for (let n = 1; n <= toplamMac; n++) {
      let ev = 0;
      try { ev = Number(Kupon.beklenenOdul(0.5, n, tur)) || 0; } catch { ev = 0; }
      if (ev > giris) enYuksekBasan = n;
    }
  }
  const taban = enYuksekBasan + 1;
  _tabanOnbellek.set(anahtar, taban);
  return taban;
}

/**
 * Puanlamaya izin verilen en az çözülen maç sayısı.
 * Yapılandırılmış oran ile ekonomik tabandan SIKI olanı kazanır: operatör
 * eşiği yükseltebilir, LC basan bölgeye indiremez.
 */
function asgariCozulen(toplamMac, tur) {
  return Math.max(
    Math.ceil(toplamMac * ASGARI_COZULEN_ORAN),
    ekonomikTaban(toplamMac, tur)
  );
}
...
async function kuponuSonuclandir(kupon, db) {
  ...
  if (!tamam && cozulen < asgariCozulen(toplamMac, kupon.tur)) {
  ...
  /**
   * ⚠️ SEZON KUPONUN, UZLAŞTIRMANIN DEĞİL.
   *
   * `Season.seasonKey()` argümansız çağrılıyordu, yani ŞU AN. Bu, `settle2`de
   * ölçülüp düzeltilen kusurun ikizi: ay sonunda kilitlenen bir kuponun
   * puanları, son maçı gece yarısını geçtiği için BİR SONRAKİ sezona
   * yazılıyordu. Uzlaştırma gecikirse (takılı maç, sunucu uykusu) sapma
   * günlerce büyüyebilir.
   *
   * ⚠️ İKİ YAZAN VARDI, BİRİ DÜZELTİLMİŞTİ. `season_totals` koleksiyonuna
   * yalnızca `routes/settle2.cjs` ve burası yazıyor; settle2 maçın kickoff'una
   * geçtiğinde burası "şimdi"de kaldı ve ikisi ayrıştı. Aynı tabloya iki
   * farklı kuralla yazmak, sıralamayı sessizce tutarsız yapar.
   *
   * ⚠️ ÇIPA `kilitISO`: kuponun giriş kapanış anı, ilk maçın kickoff'undan
   * türetiliyor (bkz. routes/kupon.cjs). Kupon bir HAFTA sürebildiği için ay
   * sınırını aşabilir; "oynandığı dönem" olarak kilit anı seçildi — son maç
   * değil, çünkü kupon o an oynanmaya başlamıştı.
   *
   * ⚠️ ETKİSİ YEREL OLARAK ÖLÇÜLEMEDİ: kuponlar Mongo birincil ve dosya
   * aynası kapalı, yani sayı veremiyorum. Bu bir TUTARLILIK düzeltmesi;
   * mekanizma settle2'de ölçülen 9 vakayla aynı.
   */
  let sezon = Season.seasonKey();
  const cipa = kupon.kilitISO ? new Date(kupon.kilitISO) : null;
  if (cipa && !Number.isNaN(cipa.getTime())) {
    const kuponSezon = Season.seasonKey(cipa);
    if (kuponSezon && kuponSezon !== sezon) {
      console.warn(`[kupon-settle] sezon sinirinda: ${kupon.id} kuponSezon=${kuponSezon} simdi=${sezon} — KUPON sezonuna yaziliyor`);
      sezon = kuponSezon;
    }
  }
  const odenemeyen = [];
...
module.exports = {
  kuponuSonuclandir, bekleyenleriSonuclandir, _sonucBul: sonucBul,
  // Sınama için: eşik hesabı. Testin kendi kopyasını yazması bu oturumda iki
  // kez yeşil-ama-ölü sonuç üretti, o yüzden gerçek fonksiyon dışa açık.
  _asgariCozulen: asgariCozulen, _ekonomikTaban: ekonomikTaban,
};
