...
const { fiksturKilidi } = require("./fikstur-kilit.cjs");

/**
 * Bahis kickoff'tan kac dakika ONCE kapansin. Varsayilan 0 = kickoff ani.
 * Asil koruma `status !== "NS"` kontrolu; bu saat tabanli yedek.
 */
const KILIT_ONCE_DK = Number(process.env.SKORLIG_POOL_KILIT_DK || 0);
...
// Pay dagitimi TEK KAYNAK: ayni yuvarlama kusuru turnuvada da vardi.
const { odemeDagit } = require("./pay-dagitim.cjs");
...
async function placeBet({ fixtureId, userId, side, amount, isBot }, db) {
  ...
  // ⚠️ BASLAMIS MACA BAHIS YOK. Bu kontrol HIC YOKTU: oyuncu canli skoru
  // gorup 89'da 2-0 olan maca kazanan tarafa bahis koyabiliyordu. Uzlasma mac
  // bitince calistigi icin arada gercek bir pencere vardi.
  const kilit = await fiksturKilidi(fid, { oncekiDk: KILIT_ONCE_DK, db });
  if (kilit.locked) {
    return { ok: false, reason: "MATCH_LOCKED", detail: kilit.reason, kickoffISO: kilit.kickoffISO || null };
  }

  /* ⚠️ TAVAN KONTROLÜ İLE BAHSİN YAZILMASI AYNI KİLİTTE OLMAK ZORUNDA.
   *
   * BULUNAN: mevcut bahis OKUNUYOR, tavan hesaplanıyor, sonra LC harcanıp
   ...
   *
   * ÖLÇÜLDÜ (bellek-içi Mongo, tavan 20, 8 istek x 20 LC):
   *     sıralı    → 1 kabul, bahis toplamı  20   (tavan tutuyor)
   *     eşzamanlı → 6-8 kabul, toplam 120-160    (tavanın 6-8 katı)
   *
   * Sıralı taban önemliydi: tavan havuzla birlikte büyüdüğü için "zaten
   * büyüyor olabilir" ihtimalini elemek gerekiyordu — sıralıda 20'de kalıyor.
   *
   * ⚠️ KİLİT (maç, kullanıcı) BAŞINA: tavan kullanıcının KENDİ toplamına
   * uygulanıyor, yani yarış yalnızca kullanıcının kendisiyle. Maç başına tek
   * kilit tüm oyuncuları sıraya sokardı. */
  return withFileLock(`pool-bet:${fid}:${lower(uid)}`, async () => {
  ...
  try {
    ...
    // ⚠️ İADENİN KENDİSİ DE BAŞARISIZ OLABİLİR. Bu dal zaten Mongo tökezlediği
    // için çalışıyor, yani iadenin de patlaması en olası senaryo. Sessiz
    // kalınırsa kullanıcı LC'yi ödemiş ama bahsi yazılmamış olur.
    const iade = await Wallet.creditLc(db, uid, tutar, "pool_bet_refund", { fixtureId: fid, reason: "write_failed" });
    if (!iade) {
      await borcKaydet(db, "pool_bet_refund", fid,
        [{ userIdLower: lower(uid), tutar }], 1);
    }
 ...
 */
/**
 * ÖDENEMEYEN HAVUZ PARASINI KALICI İZE YAZ.
 *
 * ⚠️ NEDEN: `settlePool` havuzu ödemeden ÖNCE `settledAt` ile mühürlüyor —
 * doğru sıra, çifte ödeme olmasın diye. Ama `creditLc` hatayı kendi içinde
 * yutup `false` döndüğü için başarısız ödeme yalnızca `paid` toplamını
 * düşürüyordu: kimse hangi oyuncunun ödenmediğini bilmiyor, mühür yüzünden
 * tekrar da denenmiyordu. Kupon, mini turnuva, turnuva ve bayat temizleyici
 * bu durumu `failed_awards`e yazıyor; havuz yazmıyordu.
 */
async function borcKaydet(db, kaynak, fid, odenemeyen, beklenen) {
  if (!odenemeyen.length) return;
  console.error(`[pool-store] ⛔ ODEME YAPILAMADI ${kaynak} mac=${fid} kisi=${odenemeyen.length}`);
  await Wallet.kayipOdulKaydet(db, {
    kaynak,
    fixtureId: fid,
    odemeler: odenemeyen,
    beklenen,
...
async function settlePool(fixtureId, outcome, db) {
  ...
  if (kazananToplam <= 0) {
    ...
    for (const b of bets) {
      ...
      if (t <= 0) continue;
      if (await Wallet.creditLc(db, b.userId, t, "pool_refund_no_winner", { fixtureId: fid })) iade += t;
      else odenemeyen.push({ userIdLower: String(b.userId).toLowerCase(), tutar: t });
    }
    await borcKaydet(db, "pool_refund_no_winner", fid, odenemeyen, bets.length);
  ...
  /* ⚠️ ÖDEMELER HAVUZLA UZLAŞTIRILIR — HER PAYI AYRI YUVARLAMAK LC ÜRETİYORDU.
   *
   * Eski kod kazanan başına `Math.round(bahis × çarpan)` yapıyor ve toplamı
   * havuzla hiç karşılaştırmıyordu.
   *
   ...
   *
   * ⚠️ AYNI KUSUR TURNUVADA DA VARDI ve orada düzeltilmişti (%23.4 fazla
   * ödeme). Kural o dosyada kalmıştı; havuz aynı hatayı taşımaya devam
   * ediyordu. Artık tek kaynak: lib/pay-dagitim.cjs (en büyük kalan yöntemi).
   *
   * YAKILAN ARTIK TÜRETİLİYOR: dağıtılan neyse yakılan `toplam - dağıtılan`.
   * Böylece `ödenen + yakılan === havuz` DEĞİŞMEZİ tam sağlanır — kesintiyi
   * ayrı yuvarlayıp payları ayrı yuvarlamak tam da bu değişmezi bozuyordu.
   */
  const kazananBahisler = bets.filter((b) => b.side === outcome);
  const dagitilacak = kaybedenVar ? Math.floor(toplam * (1 - CUT_PCT)) : toplam;
  const paylar = odemeDagit(
    dagitilacak,
    kazananBahisler.map((b) => Number(b.amount || 0) / kazananToplam)
  );
  const yakilan = r2(toplam - paylar.reduce((a, b) => a + b, 0));

  let odenen = 0;
  const odenemeyen = [];
  let kazananSayisi = 0;
  for (let i = 0; i < kazananBahisler.length; i++) {
    const b = kazananBahisler[i];
    const kazanc = paylar[i] || 0;
    ...
    else odenemeyen.push({ userIdLower: String(b.userId).toLowerCase(), tutar: kazanc });
  }
  await borcKaydet(db, "pool_win", fid, odenemeyen, kazananSayisi);
