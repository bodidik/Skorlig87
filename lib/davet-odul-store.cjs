...
async function odulMuhurle(invitedUserId, db) {
   ...
   *
   * BULUNAN: `ensureIndexes` hatayı loglayıp geçiyordu ve akış devam
   * ediyordu. Ama bu mührün TEK mekanizması benzersiz indeks: o yoksa
   * `insertOne` her çağrıda başarılı olur ve fonksiyon her seferinde `true`
   * döner — yani "ödülü ver" der.
   *
   * ÖLÇÜLDÜ (koleksiyonda kopya kayıt bırakıp indeks kurulumunu düşürdüm):
   *     üç çağrı → [true, true, true]
   * Yani davet ödülü (iki tarafa birden) ÜÇ KEZ ödenir. Dosyanın kendi
   * başlığı zaten "30 LC yerine 60 LC basılır" diyor.
   *
   * ⚠️ FAIL-CLOSED: sayamıyorsak ödeme yapmıyoruz. `false` dönmek "zaten
   * almış" ile aynı yol ve çağıran hiçbir şey ödemiyor — bu dosyanın kendi
   * kuralı ("belirsizse ödeme yapma"). */
  if (!(await ensureIndexes(db))) {
    console.error("[davet-odul] benzersiz indeks yok — odul verilmiyor (fail-closed)");
    return false;
  }
  try {
