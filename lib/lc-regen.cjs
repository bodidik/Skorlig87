...
function applyRegen(user, nowMs = Date.now(), opts = null) {
  ...
  /**
   * ⚠️ CAP'E ÇARPAN BİRİKİMDE ARTAN SÜRE YANAR.
   *
   * BULUNAN: `earned` cap'e takıldığında (`cap - bal` sınırı) sayaç yalnızca
   * KULLANILAN tik kadar ilerliyordu; geri kalan tikler `lastRegenAt` içinde
   * BANKALANMIŞ olarak duruyordu. Cüzdana dokunan yollar önce `applyRegen`
   * çağırıp SONRA harcama düşüyor (bkz. routes/pred.cjs), yani oyuncu cap'e
   * dolduğu istekte parasını harcayınca bir sonraki dokunuşta bankalanmış
   * süre nakde çevriliyordu.
   *
   * ÖLÇÜLDÜ (cap 15, tik 1 LC / 4 saat, 48 saat uzak kalmış, bakiye 14):
   *     1) giriş           → +1 LC  (bakiye 15) — lastRegenAt HÂLÂ 44 saat geride
   *     2) aynı istekte 15 LC harcandı → bakiye 0
   *     3) 1 saniye sonra  → +11 LC
   *     toplam 12 LC, hak edilen 1 LC
   * Yani cap fiilen anlamsızlaşıyordu: harcadıktan hemen sonra cap kadar
   * bedava dolum çekilebiliyordu.
   *
   * Kural dosyanın kendi başlığında zaten yazılı: "cap doluyken süre bankada
   * birikmez". Cap'e ÇARPARAK dolan birikim de aynı kurala tabi.
   */
  if (user.balance >= cap) {
    user.lastRegenAt = new Date(nowMs).toISOString();
  } else {
    // Cap'e değmedi: kullanılan tik kadar ilerlet, artan süre sonraki tik'e sayılır.
